from pathlib import Path

import modal


app = modal.App("soundfix-residual-restore-probe")

image = (
    modal.Image.debian_slim(python_version="3.10")
    .apt_install(
        "ffmpeg",
        "libsndfile1",
    )
    .pip_install(
        "numpy==1.26.4",
        "librosa==0.10.2.post1",
        "soundfile==0.12.1",
        "torch==2.3.0",
        "torchaudio==2.3.0",
    )
)


@app.function(
    image=image,
    gpu="L4",
    timeout=600,
)
def restore_with_residual_model(
    input_audio_bytes: bytes,
    model_bytes: bytes,
    residual_amount: float = 0.35,
) -> bytes:
    import numpy as np
    import soundfile as sf
    import librosa
    import torch
    import torch.nn as nn
    from tempfile import TemporaryDirectory

    class Simple1D(nn.Module):
        def __init__(self):
            super().__init__()
            self.net = nn.Sequential(
                nn.Conv1d(1, 32, 9, padding=4),
                nn.ReLU(),
                nn.Conv1d(32, 64, 9, padding=4),
                nn.ReLU(),
                nn.Conv1d(64, 32, 9, padding=4),
                nn.ReLU(),
                nn.Conv1d(32, 1, 9, padding=4),
            )

        def forward(self, x):
            return self.net(x)

    def peak_normalize(audio: np.ndarray, target: float = 0.95) -> np.ndarray:
        peak = float(np.max(np.abs(audio))) if audio.size > 0 else 0.0

        if peak > 0:
            audio = audio / peak * target

        return np.clip(audio, -1.0, 1.0).astype(np.float32)

    sample_rate = 22050

    with TemporaryDirectory() as temp_dir:
        temp_path = Path(temp_dir)
        input_path = temp_path / "input.wav"
        model_path = temp_path / "residual_model.pt"
        output_path = temp_path / "residual_restored.wav"

        input_path.write_bytes(input_audio_bytes)
        model_path.write_bytes(model_bytes)

        audio, _ = librosa.load(input_path, sr=sample_rate)
        audio = audio.astype(np.float32)

        device = "cuda" if torch.cuda.is_available() else "cpu"

        model = Simple1D().to(device)
        model.load_state_dict(torch.load(model_path, map_location=device))
        model.eval()

        with torch.no_grad():
            x = torch.from_numpy(audio).float().to(device)
            x = x.unsqueeze(0).unsqueeze(0)

            predicted_residual = model(x)
            predicted_residual = predicted_residual.squeeze().detach().cpu().numpy().astype(np.float32)

        restored = audio + predicted_residual * float(residual_amount)
        restored = peak_normalize(restored)

        sf.write(output_path, restored, sample_rate, format="WAV")

        print(
            {
                "output": str(output_path),
                "sample_rate": sample_rate,
                "duration_seconds": float(restored.shape[0] / sample_rate),
                "residual_amount": float(residual_amount),
                "device": device,
            },
            flush=True,
        )

        return output_path.read_bytes()


@app.local_entrypoint()
def main(
    input_path: str = "backend/test_acapella_ineedyourlove.mp3",
    model_path: str = "backend/residual_model.pt",
    output_path: str = "residual_restored_ineedyourlove.wav",
    residual_amount: float = 0.35,
):
    with open(input_path, "rb") as input_file:
        input_audio_bytes = input_file.read()

    with open(model_path, "rb") as model_file:
        model_bytes = model_file.read()

    output_audio_bytes = restore_with_residual_model.remote(
        input_audio_bytes,
        model_bytes,
        residual_amount,
    )

    with open(output_path, "wb") as output_file:
        output_file.write(output_audio_bytes)

    print(f"Saved output to {output_path}")