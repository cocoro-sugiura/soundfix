...
image = (
    ...
    .run_commands(
        ...
        "python -c \"from pathlib import Path; p=Path('/opt/sgmse/enhancement.py'); t=p.read_text(); t=t.replace('write(join(args.enhanced_dir, filename), x_hat.cpu().numpy(), target_sr)', 'write(join(args.enhanced_dir, filename), x_hat.squeeze().cpu().numpy(), target_sr, format=\\\"WAV\\\")'); p.write_text(t)\"",
