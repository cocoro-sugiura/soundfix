...
image = (
    ...
    .run_commands(
        ...
        "python -c \"from pathlib import Path; p=Path('/opt/sgmse/enhancement.py'); t=p.read_text(); t=t.replace('from soundfile import write', 'from soundfile import write\\nimport torch\\nimport torchaudio'); t=t.replace('write(join(args.enhanced_dir, filename), x_hat.cpu().numpy(), target_sr)', 'torchaudio.save(join(args.enhanced_dir, filename), x_hat.cpu().float().reshape(1, -1), target_sr)'); p.write_text(t)\"",
